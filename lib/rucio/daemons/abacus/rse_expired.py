...
def run_once(
    ...
    for rse in rses:
        ...
        logger(logging.DEBUG, '%s replica backlog query for RSE %s took %f s.' % ('Deletable' if not obsolete else 'Obsolete', rse['id'], time.time() - start))
...
def run(
    ...
    """
    global DAEMON_NAME
    setup_logging(process_name=DAEMON_NAME if not obsolete else 'abacus-rse-obsolete')
    ...
    if once:
        ...
        thread_list = [threading.Thread(target=rse_update, kwargs={'once': once, 'obsolete': obsolete, 'sleep_time': sleep_time})]
